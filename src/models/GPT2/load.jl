...
using ..TransformerLayers: CompositeEmbedding, SelfAttention, CausalMultiheadQKVAttenOp, @fluxshow, @fluxlayershow
using Functors
using Static
...
struct HGFGPT2Model{E,D} <: HGFPreTrained{:gpt2,:model}
    embed::E
    decoder::D
end
...
struct HGFGPT2LMHeadModel{M,C} <: HGFPreTrained{:gpt2,:lmheadmodel}
    model::M
    cls::C
end
...
function load_model(_type::Type{HGFGPT2LMHeadModel}, cfg, state_dict, prefix)
    ...
    return HGFGPT2LMHeadModel(model, TransformerLayers.Branch{(:logit,),(:hidden_state,)}(lmhead))
end
...
function load_model(_type::Type{<:HGFGPT2PreTrainedModel}, ::Type{<:TransformerLayers.LayerNorm}, cfg, state_dict, prefix)
    dims = cfg[:n_embd]
...
function load_model(_type::Type{<:HGFGPT2PreTrainedModel}, ::Type{<:SelfAttention}, cfg, state_dict, prefix)
    ...
    attn_weight = getweight(weight_init(dims, 3dims, factor), TransformerLayers.Embed, state_dict, joinname(prefix, "c_attn.weight"))
    attn_bias = getweight(zero_init(3dims), Array, state_dict, joinname(prefix, "c_attn.bias"))
    qkv_proj = TransformerLayers.NSplit(static(3), TransformerLayers.Dense(attn_weight, attn_bias))
    o_weight = getweight(weight_init(dims, dims, factor), TransformerLayers.Embed, state_dict, joinname(prefix, "c_proj.weight"))
...
function load_model(
    _type::Type{<:HGFGPT2PreTrainedModel}, ::Type{<:TransformerLayers.Chain{<:Tuple{TransformerLayers.Dense,TransformerLayers.Dense}}},
    ...
    wi_weight = getweight(weight_init(dims, ff_dims, factor), TransformerLayers.Embed,
        state_dict, joinname(prefix, "c_fc.weight"))
    wi_bias = getweight(zero_init(ff_dims), Array, state_dict, joinname(prefix, "c_fc.bias"))
    wo_weight = getweight(weight_init(ff_dims, dims, factor), TransformerLayers.Embed,
...
function get_state_dict(p::Type{<:HGFGPT2PreTrainedModel}, m::TransformerLayers.Dense, state_dict, prefix)
...
function get_state_dict(p::Type{<:HGFGPT2PreTrainedModel}, m::TransformerLayers.Chain{<:Tuple{TransformerLayers.Dense,TransformerLayers.Dense}},
    state_dict, prefix)
