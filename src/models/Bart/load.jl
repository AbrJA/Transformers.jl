...
using ..TransformerLayers: CompositeEmbedding, SelfAttention, CrossAttention, MultiheadQKVAttenOp, CausalMultiheadQKVAttenOp, @fluxshow, @fluxlayershow
using Functors
...
function load_model(::Type{<:HGFBartPreTrainedModel}, ::Type{TransformerLayers.Embed}, cfg, state_dict, prefix)
    ...
    token_weight = getweight(TransformerLayers.Embed, state_dict, joinname(prefix, "weight")) do
        weight = weight_init(vocab_size, dims, factor)()
        weight[:, pad_id+1] .= 0
        return weight
    end
    return TransformerLayers.Embed(scale, token_weight)
end
function load_model(::Type{<:HGFBartPreTrainedModel}, ::Type{TransformerLayers.FixedLenPositionEmbed}, cfg, state_dict, prefix)
    dims = cfg[:d_model]
    max_pos = cfg[:max_position_embeddings]
    factor = Float32(cfg[:init_std])
    pos_weight = getweight(weight_init(max_pos + 2, dims, factor), #= following HGF Bart's hack =#
        TransformerLayers.Embed, state_dict, joinname(prefix, "embed_positions.weight"))
    return TransformerLayers.FixedLenPositionEmbed(pos_weight)
end

function load_model(_type::Type{<:HGFBartPreTrainedModel}, ::Type{CompositeEmbedding}, cfg, state_dict, prefix)
    dims = cfg[:d_model]
    token_embed = load_model(_type, TransformerLayers.Embed, cfg, state_dict, joinname(prefix, "shared"))
    enc_pos = load_model(_type, TransformerLayers.FixedLenPositionEmbed, cfg, state_dict, joinname(prefix, "encoder"))
    dec_pos = load_model(_type, TransformerLayers.FixedLenPositionEmbed, cfg, state_dict, joinname(prefix, "decoder"))
    ...
    return TransformerLayers.Parallel{(:encoder_input, :decoder_input)}((
        TransformerLayers.Chain(enc_embed, enc_ln),
        TransformerLayers.Chain(dec_embed, dec_ln)))
end

load_model(::Type{<:HGFBartPreTrainedModel}, ::Type{TransformerLayers.LayerNorm}, cfg, state_dict, prefix) =
...
function load_model(
    ...
    p = Float64(cfg[:attention_dropout])
...
function load_model(::Type{<:HGFBartPreTrainedModel}, ::Type{TransformerLayers.CrossAttention}, cfg, state_dict, prefix)
    ...
    kv_proj = TransformerLayers.Fork(
        _load_dense(state_dict, joinname(prefix, "k_proj"), dims, dims, factor, true),
        _load_dense(state_dict, joinname(prefix, "v_proj"), dims, dims, factor, true),
    )
    o_proj = _load_dense(state_dict, joinname(prefix, "out_proj"), dims, dims, factor, true)
...
function _load_bart_ffn(state_dict, prefix, dims, ff_dims, factor, p, act)
    ...
    return TransformerLayers.Chain(TransformerLayers.DropoutLayer(fc1, p), fc2)
end
...
function load_model(_type::Type{<:HGFBartPreTrainedModel}, ::Type{<:TransformerBlock}, cfg, state_dict, prefix)
    ...
    factor = Float32(cfg[:init_std])
    act_p = Float64(cfg[:activation_dropout])
    act_p = iszero(act_p) ? nothing : act_p
    act = ACT2FN[Symbol(cfg[:activation_function])]
    n = cfg[:encoder_layers]
    p = Float64(cfg[:dropout])
    p = iszero(p) ? nothing : p
    ...
    for i = 1:n
        ...
        sa_ln = load_model(_type, TransformerLayers.LayerNorm, cfg, state_dict, joinname(lprefix, "self_attn_layer_norm"))
        ...
        ff_ln = load_model(_type, TransformerLayers.LayerNorm, cfg, state_dict, joinname(lprefix, "final_layer_norm"))
...
function load_model(_type::Type{<:HGFBartPreTrainedModel}, ::Type{<:TransformerDecoderBlock}, cfg, state_dict, prefix)
    ...
    for i = 1:n
        ...
        ca_ln = load_model(_type, TransformerLayers.LayerNorm, cfg, state_dict, joinname(lprefix, "encoder_attn_layer_norm"))
        ca = TransformerLayers.PostNormResidual(TransformerLayers.DropoutLayer(ca, p), ca_ln)
        ff = _load_bart_ffn(state_dict, lprefix, dims, ff_dims, factor, act_p, act)
        ff_ln = load_model(_type, TransformerLayers.LayerNorm, cfg, state_dict, joinname(lprefix, "final_layer_norm"))
        ff = TransformerLayers.PostNormResidual(TransformerLayers.DropoutLayer(ff, p), ff_ln)
...
function get_state_dict(p::Type{<:HGFBartPreTrainedModel}, m::TransformerLayers.Parallel, state_dict, prefix)
...
function get_state_dict(p::Type{<:HGFBartPreTrainedModel}, m::TransformerLayers.Chain{<:Tuple{CompositeEmbedding,Any}}, state_dict, prefix)
