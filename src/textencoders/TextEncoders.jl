...
function set_field(e::TrfTextEncoder, field::Symbol, val)
    fields = ntuple(fieldcount(TrfTextEncoder)) do i
        f = fieldname(TrfTextEncoder, i)
        f === field ? val : getfield(e, f)
    end
    return TrfTextEncoder(fields...)
end
    ...
    @eval $(
        quote
            ...
            function $(Symbol(:set_, name))(builder, e::TrfTextEncoder)
                set_field(e, $(QuoteNode(name)), builder(e))
            end
        end
    )
end
...
function set_tokenizer(builder, e::TrfTextEncoder)
    ...
    return set_field(e, :tokenizer, new_tkr)
end
